!function (a) {
  ...
  function g (a, b) {
    ...
    function k (m) {
      m.each(function () {
        ...
        var A, B, C, D = k.g = a.select(this), E = this.__chart__ || p, F = this.__chart__ = g(), G = t ? t : f(F),
          H = D.selectAll('.tick').data(G, F),
          I = H.enter().insert('g', '.domain').attr('class', 'tick').style('opacity', 1e-6), J = H.exit().remove(),
          K = j(H).style('opacity', 1), L = p.rangeExtent ? p.rangeExtent() : e(p.range()),
          M = D.selectAll('.domain').data([0]), N = (M.enter().append('path').attr('class', 'domain'), j(M))
  ...
  k.generate = function (a) {
    ...
    var b = this, c = [b.getXDomainMin(a), b.getXDomainMax(a)], d = c[0], e = c[1], f = b.getXDomainPadding(c), g = 0,
      h = 0
    ...
    var b, c, d = this, e = d.config, f = d.classChartLine.bind(d), g = d.classLines.bind(d), h = d.classAreas.bind(d),
    ...
    var c = this, d = b ? c.subXAxis : c.xAxis, e = a.__max__ + 1, f = c.getBarW(d, e), g = c.getShapeX(f, e, a, !!b),
      h = c.getShapeY(!!b), i = c.getShapeOffset(c.isBarType, a, !!b), j = b ? c.getSubYScale : c.getYScale
    return function (a, b) {
    ...
    var b = this.d3.mouse(a), c = a.getBoundingClientRect(), d = a.pathSegList.getItem(0), e = a.pathSegList.getItem(1),
      f = Math.min(d.x, e.x), g = Math.min(d.y, e.y), h = c.width, i = c.height, j = 2, k = f - j, l = f + h + j,
      m = g + i + j, n = g - j
    ...
    function e (b, c, e) {
      ...
      var g, h, i = 0 === e, j = e === a.length - 1, k = d(b, c),
        l = k.width + F + (!j || y.isLegendRight || y.isLegendInset ? B : 0) + z.legend_padding, m = k.height + A,
    ...
    d = h.selectAll('.' + l.arcs).selectAll('.' + l.arc).data(e.arcData.bind(e)), d.enter().append('path').attr('class', e.classArc.bind(e)).style('fill', function (a) {
      ...
      var a = {data: [{value: g.gauge_max}], startAngle: g.gauge_startingAngle, endAngle: -1 * g.gauge_startingAngle}
  ...
  h.focus = function (a) {
    ...
    return function () {
      var e, f, g, h = a.targets, i = a.flow, j = a.drawBar, k = a.drawLine, m = a.drawArea, n = a.cx, o = a.cy,
        ...
        x = b.getValueOnIndex(b.data.targets[0].values, v), y = b.getValueOnIndex(b.data.targets[0].values, v + w),
        z = b.x.domain(), A = i.duration || s, B = i.done || function () {
        }, C = b.generateWait(), D = b.xgrid || d.selectAll([]), E = b.xgridLines || d.selectAll([]),
        F = b.mainRegion || d.selectAll([]), G = b.mainText || d.selectAll([]), H = b.mainBar || d.selectAll([]),
        I = b.mainLine || d.selectAll([]), J = b.mainArea || d.selectAll([]), K = b.mainCircle || d.selectAll([])
    ...
    f.data_selection_enabled && d.main.selectAll('.' + l.shapes).selectAll('.' + l.shape).each(function (g, h) {
      var i = e.select(this), j = g.data ? g.data.id : g.id, k = d.getToggle(this, g).bind(d),
        m = f.data_selection_grouped || !a || a.indexOf(j) >= 0, n = !b || b.indexOf(h) >= 0, o = i.classed(l.SELECTED)
    ...
    e.data_selection_enabled && c.main.selectAll('.' + l.shapes).selectAll('.' + l.shape).each(function (f, g) {
      var h = d.select(this), i = f.data ? f.data.id : f.id, j = c.getToggle(this, f).bind(c),
        k = e.data_selection_grouped || !a || a.indexOf(i) >= 0, m = !b || b.indexOf(g) >= 0, n = h.classed(l.SELECTED)
